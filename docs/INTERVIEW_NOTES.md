...
## Can the harness integrate another AI service?

Yes. The integration boundary is modular. For a conventional JSON API, I can use the built-in `generic_http` adapter and register the service with its health endpoint, run endpoint, capabilities and evaluation pack. For an asynchronous or non-standard service, I implement a custom adapter that maps its execution and telemetry into the standard `AgentRun` contract.

I then select or add a reusable evaluation pack, attach a versioned golden dataset, define quality gates, and the existing evaluation, regression, observability, audit and dashboard layers continue to work without being rewritten.

The application itself does not have to import the harness; the harness integrates at the service boundary.


## Advanced LLMOps extensions

- Experiment comparison: compare baseline vs candidate batches on the same golden set and classify metric regressions/improvements using metric directionality.
- Agent trace viewer: persist normalized step traces and run metadata for debugging tool-call sequences, approvals and failures.
- Prompt/model/config registry: carry version lineage with evaluation and target runs so changes are attributable.
- CI quality gate: fail CI when evaluation gates fail or reference scores regress beyond threshold.
- LLM-as-a-judge: optional shared-gateway qualitative judge for relevance, groundedness, completeness and faithfulness; skipped when no provider session exists rather than fabricating results.
